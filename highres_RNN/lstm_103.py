...
H_t = DataFunctions.fix_data(Elev) #substract to find differences
...
#train_data = numpy.concatenate((X_sch_t, T_sch_t), axis=1)
train_data = X_sch_t
...
if choice == 1:
    H_t = numpy.load('Ht_file_Elev.npy')
else:
    ...
    numpy.save('Ht_file_Elev.npy', H_t)

H_rms = MathFunctions.rms_flat(H_t)
#Saving to txt for FFT
numpy.savetxt('H_t.csv', H_t, delimiter=',')

#PlotFunctions.Plot_double(numpy.arange(0, 20), H_t[0:20], numpy.arange(0, 20), H_t[0:20], 'Actual conv power','LSTM conv power', 'k-', 'r-', "rough.eps")
...
###One hot encoding
S, unique, pk = DataFunctions.calculate_entropy(H_t)
J_t = DataFunctions.convert_to_discrete(H_t, unique)
J_t = DataFunctions.one_hot_encoder_101(J_t)
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_val = DataFunctions.fix_data(Elev) #substract to find differences
...
# Getting timescale features
T_sch_val = TimeScaleFunctions.timescale_101(X_sch_val, date_start, ref_date, timescales)

print X_sch_val.shape
print X_sch_val[0]


#val_data = numpy.concatenate((weather_val[:, 0:2], X_sch_val, T_sch_val), axis=1)
#val_data = numpy.concatenate((X_sch_val, T_sch_val), axis=1)
val_data = X_sch_val
...
if choice == 1:
    H_val = numpy.load('Hv_file_Elev.npy')
else:
    ...
    numpy.save('Hv_file_Elev.npy', H_val)
...
### One hot encoding
#S, unique, pk = DataFunctions.calculate_entropy(H_val)
J_val = DataFunctions.convert_to_discrete(H_val, unique)
J_val = DataFunctions.one_hot_encoder_101(J_val)
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_e = DataFunctions.fix_data(Elev) #substract to find differences
...
# Getting timescale features
T_sch_e = TimeScaleFunctions.timescale_101(X_sch_e, date_start, ref_date, timescales)
...
#combining features
#test_data = numpy.concatenate((weather_test[:, 0:2], X_sch_e, T_sch_e), axis=1)
#test_data = numpy.concatenate((X_sch_e, T_sch_e), axis=1)
test_data = X_sch_e

test_data = DataFunctions.normalize_2D(test_data)

if choice == 1:
    H_e = numpy.load('He_file_Elev.npy')
else:
    ...
    numpy.save('He_file_Elev.npy', H_e)
...
#####One hot Encoding
#S, unique, pk = DataFunctions.calculate_entropy(H_e)
J_e = DataFunctions.convert_to_discrete(H_e, unique)
J_e = DataFunctions.one_hot_encoder_101(J_e)
#Implement daily network here

#  `
#print H_mean_t.shape
label_num = len(unique)
...
#Expressing H values as tensor
#H_t = numpy.reshape(H_t, (H_mean_t.shape[0], 24, 1))
#H_val = numpy.reshape(H_val, (H_mean_v.shape[0], 24, 1))
#H_e = numpy.reshape(H_e, (H_mean_e.shape[0], 24, 1))

#Reshaping J values
J_t = numpy.reshape(J_t, (H_mean_t.shape[0], 24, label_num))
J_val = numpy.reshape(J_val, (H_mean_v.shape[0], 24, label_num))
J_e = numpy.reshape(J_e, (H_mean_e.shape[0], 24, label_num))
...
space = {
        ...
        #'Layer3': hp.qloguniform('Layer3', 1, 5, 1),
        'Layer3': hp.quniform('Layer3', 5, 20, 1), #change this [5, 20, 1]
...
def objective(params):
    ...
    optimize_model = build_lstm_v1.lstm_model_107(params, train_data.shape[2], 24, len(unique))

    #for epochs in range(5):
    for ep in range(5):
        #optimize_history = optimize_model.fit(X_seq, Y_seq, batch_size=1, nb_epoch=3, validation_split=(X_seq, Y_seq), shuffle=False)
        #optimize_history = optimize_model.fit(train_data, J_t, batch_size=1, nb_epoch=1, validation_data=(val_data, J_val), shuffle=False)
...
lstm_model = build_lstm_v1.lstm_model_107(lstm_hidden, train_data.shape[2], tsteps, len(unique))
...
for attempts in range(attempt_max):
    ...
    for ep in range(epoch_max):
        #lstm_history = lstm_model.fit(train_data, J_t, batch_size=1, nb_epoch=1, validation_data=(val_data, J_val), shuffle=False, callbacks=callbacks)
        ...
        e1, e2 = DataFunctions.find_error(H_t, H_t, y_val)
        #print e1, e2
        val_loss_check = e2
        val_loss_v.append(val_loss_check)
        e_mat[ep, attempts] = val_loss_check
        ...
        if val_loss_v[count] < epsilon:
            ...
            #Y_lstm = save_model.predict(test_data, batch_size=1, verbose=0)
...
#Y_lstm3 = DataFunctions.convert_to_continous_3D(Y_lstm2, unique)
...
#setting to discrete values
#Y_lstm2 = DataFunctions.fix_discrete(Y_lstm2, unique)
...
best_NN = NNFunctions.NN_optimizeNN_v22(X1, H1, X2, H2)
...
#### Plotting
PlotFunctions.Plot_double(t_array, H_e, t_array, Y_lstm2, 'Actual conv power','LSTM conv power', 'k-', 'r-', "fig_Elev1a.eps")

PlotFunctions.Plot_triple(t_train, H_t, t_test, Y_lstm2, t_test, H_e, 'Training Data', 'LSTM predictions', 'Test Data (actual)', 'k-', 'r-', 'b-', "fig_Elev1b.eps")

#PlotFunctions.Plot_triple(t_train, H_t, t_test, Y_lstm, t_test, H_e, 'Training Data', 'LSTM predictions', 'Test Data (actual)', 'k-', 'r-', 'b-', "fig_crac1c.eps")
