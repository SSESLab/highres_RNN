...
def objective(params):
    optimize_model = build_lstm_v1.lstm_multi_107b(params, train_data2.shape[2],  lstm_h1, (std_inv/std_inv2)) #Check code here, relu entering
...
#Building model
lstm_model = build_lstm_v1.lstm_multi_107b(lstm_hidden, train_data2.shape[2],  lstm_h1, tsteps2)
save_model = lstm_model
...
for attempts in range(attempt_max):
    ...
    for ep in range(epoch_max):
        lstm_history = lstm_model.fit([train_data2, h_input], H_t2, batch_size=1, nb_epoch=1, validation_data=([val_data2, h_val], H_val2), shuffle=False)
        loss_old = loss_val
        loss_val = lstm_history.history['loss']
        ...
        if val_loss_v[count] < epsilon and loss_val < loss_old:
            ...
            Y_val = test_model.predict([val_data2, h_val], batch_size=1, verbose=0)
            test_model.reset_states()
...
numpy.save('CRAC_normal_10min2C0.npy', Y_lstm)
numpy.save('CRAC2C_val0', Y_val)
