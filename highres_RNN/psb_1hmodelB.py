...
numpy.save('X1_HVAC1.npy', X1)
numpy.save('S1_HVAC1.npy', X_sch_t2)
...
space = {
        ...
        'Layer3': hp.quniform('Layer3', 5, 20, 1),
         ...
         'activ_l4': hp.choice('activ_l4', ['linear', 'sigmoid'])
         }
...
def objective(params):
    optimize_model = build_lstm_v1.lstm_model_110(params, train_data2.shape[2],  (std_inv/std_inv2)) #Check code here, relu entering
...
trials = Trials()
best2 = fmin(objective, space, algo=tpe.suggest, trials=trials, max_evals=20)

#Building Stateful Model
lstm_hidden = hyperopt.space_eval(space, best2)
print lstm_hidden
tsteps2 = std_inv/std_inv2

#Building model
lstm_model = build_lstm_v1.lstm_model_110(lstm_hidden, train_data2.shape[2],  tsteps2)
...
for attempts in range(attempt_max):
    ...
    for ep in range(epoch_max):
        ...
        if val_loss_v[count] < epsilon and loss_val < loss_old:
            ...
            test_model.reset_states()
            #Y_val = DataFunctions.fix_bindata(Y_val)
            #Y_val= DataFunctions.make_realdata(Y_val, delta)
...
numpy.save('CRAC2_1h.npy', Y_lstm)
#numpy.save('HVAC2_1h.npy', Y_val)
...
#numpy.save('HVAC_critical_10minMLP.npy', Y_NN)
