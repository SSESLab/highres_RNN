...
delta = 0.02439024 #delta for BG


#Read Data
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
boiler_gas, hot_water_gas, total_gas = DataFunctions.PSB_heat_split(data) #split by end uses
H_t2 = DataFunctions.prepare_energy_PSB(total_gas, std_inv2) #Energy Consumption Data for multi timescale analysis
...
if choice == 1:
    H_t = numpy.load('H1_file_TG1.npy')
elif choice == 2:
    ...
    H_t = Y_t.copy()
    #H_t = Y_NN.copy()
    numpy.save('H1_file_TG1.npy', H_t)
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
boiler_gas, hot_water_gas, total_gas = DataFunctions.PSB_heat_split(data) #split by end uses
H_val2 = DataFunctions.prepare_energy_PSB(total_gas, std_inv2) #Energy Consumption Data for multi timescale analysis
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
boiler_gas, hot_water_gas, total_gas = DataFunctions.PSB_heat_split(data) #split by end uses
H_e2 = DataFunctions.prepare_energy_PSB(total_gas, std_inv2) #Energy Consumption Data for multi timescale analysis
...
numpy.save('Ht2_TG1.npy', H1)
numpy.save('Hv2_TG1.npy', H2)
numpy.save('He2_TG1.npy', H3)
...
numpy.save('TG_10min2A.npy', Y_lstm)
numpy.save('TG_val.npy', Y_val)
