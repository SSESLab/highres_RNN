...
H_t2 = DataFunctions.prepare_energy_PSB(crac_critical, std_inv2) #Energy Consumption Data for multi timescale analysis
...
if choice == 1:
    H_t = numpy.load('H1_file_CRAC1.npy')
elif choice == 2:
...
###Filling in data using H_t
E_t2 = DataFunctions.convert_to_10min(H_t*H_max, (std_inv/std_inv2))
E_t2 = E_t2/H_max2
E_t2 = DataFunctions.make_10min_tensor(E_t2, max_unit, delta)
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_val2 = DataFunctions.prepare_energy_PSB(crac_critical, std_inv2) #Energy Consumption Data for multi timescale analysis
...
T_sch_val2 = DataFunctions.get_feature_low_res(T_sch_val, std_inv, std_inv2)
T_sch_val2 = T_sch_val2[:, 0:2]

val_data = numpy.concatenate((weather_val[:, 0:2], X_sch_val, T_sch_val), axis=1)
val_data2 = numpy.concatenate((weather_val2[:, 0:2], X_sch_val2, T_sch_val2), axis=1)
#val_data = X_sch_val
#val_data2 = X_sch_val2
...
###Filling in data using H_t
E_val2 = DataFunctions.convert_to_10min(H_val*H_max, (std_inv/std_inv2))
E_val2 = E_val2/H_max2
E_val2 = DataFunctions.make_10min_tensor(E_val2, max_unit, delta)
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_e2 = DataFunctions.prepare_energy_PSB(crac_critical, std_inv2) #Energy Consumption Data for multi timescale analysis
...
T_sch_e2 = DataFunctions.get_feature_low_res(T_sch_e, std_inv, std_inv2)
T_sch_e2 = T_sch_e2[:, 0:2]

test_data = numpy.concatenate((weather_test[:, 0:2], X_sch_e, T_sch_e), axis=1)
test_data2 = numpy.concatenate((weather_test2[:, 0:2], X_sch_e2, T_sch_e2), axis=1)
#test_data = X_sch_e
#test_data2 = X_sch_e2
...
#numpy.save('Ht2_CONV2.npy', H1)
#numpy.save('Hv2_CONV2.npy', H2)
#numpy.save('He2_CONV2.npy', H3)

###Filling in data using H_t
y_B = numpy.load('CRAC_critical_B.npy')
y_temp = y_B.flatten()
print y_temp.shape

E_e2 = DataFunctions.convert_to_10min(y_temp*H_max, (std_inv/std_inv2))
E_e2 = E_e2/H_max2
E_e2 = DataFunctions.make_10min_tensor(E_e2, max_unit, delta)
...
E_t2 = numpy.reshape(E_t2, (conv_hour_to_day*H_mean_t.shape[0], int(std_inv/std_inv2), E_t2.shape[1]))
E_val2 = numpy.reshape(E_val2, (conv_hour_to_day*H_mean_v.shape[0], int(std_inv/std_inv2), E_val2.shape[1]))
E_e2 = numpy.reshape(E_e2, (conv_hour_to_day*H_mean_e.shape[0], int(std_inv/std_inv2), E_e2.shape[1]))
...
space2 = {
        ...
        'lambda_1': hp.quniform('lambda_1', 0.6, 1, 0.1)

         }


def objective(params):
    optimize_model = build_lstm_v1.lstm_multi_106(params, train_data2.shape[2], H_t2.shape[2], (std_inv/std_inv2)) #Check code here, relu entering
    loss_out = NNFunctions.model_optimizer_101(optimize_model, train_data2, [H_t2, E_t2], val_data2, [H_val2, E_val2], 5)
...
#Building model
lstm_model = build_lstm_v1.lstm_multi_106(lstm_hidden, train_data2.shape[2], H_t2.shape[2], tsteps2)
...
for attempts in range(attempt_max):
    ...
    for ep in range(epoch_max):
        lstm_history = lstm_model.fit(train_data2, [H_t2, E_t2], batch_size=1, nb_epoch=1, validation_data=(val_data2, [H_val2, E_val2]), shuffle=False)
        loss_old = loss_val
        ...
        if val_loss_v[count] < epsilon and loss_val < loss_old:
            ...
            #Y_lstm = DataFunctions.fix_bindata(Y_lstm[0, :, :, :])
...
####Saving block
numpy.save('CRAC_critical_10min2F.npy', Y_lstm)
...
#numpy.save('fig_hvac2_mlp.npy', Y_NN)
