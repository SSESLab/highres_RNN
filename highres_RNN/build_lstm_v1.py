...
def lstm_model_106b(lstm_hidden, in_dim, tsteps):
    ...
    x = Reshape((tsteps,))(x)
    x = (Dense(lstm_h3, activation='relu'))(x)
    out_3D = (Dense(tsteps, activation='sigmoid'))(x)
...
def lstm_model_109c(lstm_hidden, in_dim, tsteps):
    ...
    #inputs = [None, tsteps, in_dim]
    inputs = Input(shape=(tsteps, in_dim), batch_shape=(1, tsteps, in_dim))
    encoder = LSTM(lstm_h1, activation=activ_l1, return_sequences=False, stateful=True)(inputs)

    #Adding a dense layer
    #encoder = Dense(lstm_h1, activation='relu')(encoder)
    encoding_repeat = RepeatVector(tsteps)(encoder) #[None, in_dim] -> [None, tsteps, in_dim]
    decoder = LSTM(lstm_h2, return_sequences=True, activation=activ_l1, W_regularizer=l2(0.01), stateful=True)(encoding_repeat)
    #decoder = ActivityRegularization(l2=0.01)(decoder)
    decoder = merge([decoder, inputs], mode='concat', concat_axis=2) #THis is what I want to input
    #sequence_prediction = TimeDistributed(Dense(1,  activation='linear'))(decoder)
...
def lstm_model_110(lstm_hidden, in_dim, tsteps):
    ...
    #inputs = [None, tsteps, in_dim]
    inputs = Input(shape=(tsteps, in_dim), batch_shape=(1, tsteps, in_dim))
    encoder = LSTM(lstm_h1, activation='hard_sigmoid', W_regularizer=l2(0.0), return_sequences=False, stateful=True)(inputs)
...
def lstm_model_110b(lstm_hidden, in_dim, tsteps):
    ...
    #inputs = [None, tsteps, in_dim]
    inputs = Input(shape=(tsteps, in_dim), batch_shape=(1, tsteps, in_dim))
    encoder = LSTM(lstm_h1, activation='hard_sigmoid', return_sequences=False, W_regularizer=l2(0.01), stateful=True)(inputs)
...
def lstm_multi_101b(lstm_hidden, in_dim, h2_dim, tsteps):
    ...
    #inputs = [None, tsteps, in_dim]
    input_1 = Input(shape=(tsteps, in_dim), batch_shape=(1, tsteps, in_dim))
    encoder = LSTM(lstm_h1, activation='hard_sigmoid', return_sequences=False, stateful=True)(input_1)
    ...
    #Creatinbg graph for Multi-layer perceptron
    input_MLP = Input(shape=(tsteps, lstm_h2+in_dim+h2_dim), batch_shape=(1, tsteps, lstm_h2+in_dim+h2_dim))
    x = TimeDistributed(Dense(lstm_h3,  activation=activ_l3))(input_MLP)
    predictions_MLP = TimeDistributed(Dense(1, activation=activ_l4))(x)
    model_MLP = Model(input=input_MLP, output=predictions_MLP)
...
def lstm_multi_102(lstm_hidden1, lstm_hidden2, in_dim, h2_dim, tsteps1, tsteps2):

    #MODEL A

    lstm_h1_a = int(lstm_hidden1['Layer1'])
    lstm_h2_a = int(lstm_hidden1['Layer2'])
    lstm_h3_a = int(lstm_hidden1['Layer3'])
    activ_l3_a = lstm_hidden1['activ_l3']
    activ_l4_a = lstm_hidden1['activ_l4']

    batch_size = 1
    inputs_a = Input(shape=(tsteps1, in_dim), batch_shape=(1, tsteps1, in_dim))
    encoder_a = LSTM(lstm_h1_a, activation='tanh', return_sequences=False, stateful=True)(inputs_a)

    #Adding a dense layer
    #encoder = Dense(lstm_h1, activation='relu')(encoder)
    encoding_repeat = RepeatVector(tsteps1)(encoder_a) #[None, in_dim] -> [None, tsteps, in_dim]
    decoder_a = LSTM(lstm_h2_a, return_sequences=True, activation='tanh', W_regularizer=l2(0.0), stateful=True)(encoding_repeat)
    decoder_a = merge([decoder_a, inputs_a], mode='concat', concat_axis=2) #THis is what I want to input

    #Output of model A
    #Creatinbg graph for Multi-layer perceptron
    input_MLP = Input(shape=(tsteps1, lstm_h2_a+in_dim), batch_shape=(1, tsteps1, lstm_h2_a+in_dim))
    x = TimeDistributed(Dense(lstm_h3_a,  activation=activ_l3_a))(input_MLP)
    predictions_MLP = TimeDistributed(Dense(1, activation=activ_l4_a))(x)
    model_a = Model(input=input_MLP, output=predictions_MLP)

    #output A
    pred_a = model_a(decoder_a)
    out_a = Reshape((tsteps1, ))(pred_a) #might have to reshape


    #MODEL B

    lstm_h1_b = int(lstm_hidden2['Layer1'])
    lstm_h2_b = int(lstm_hidden2['Layer2'])
    lstm_h3_b = int(lstm_hidden2['Layer3'])
    activ_l3_b = lstm_hidden2['activ_l3']
    activ_l4_b = lstm_hidden2['activ_l4']

    # inputs = [None, tsteps, in_dim]
    inputs_b = Input(shape=(tsteps1, in_dim), batch_shape=(1, tsteps2, in_dim))
    encoder_b = LSTM(lstm_h1_b, activation='tanh', return_sequences=False, stateful=True)(inputs_b)

    return None



def lstm_multi_103(lstm_hidden, in_dim, h1_dim, tsteps):
...
def lstm_multi_104(lstm_hidden, in_dim, out_dim, tsteps):

    lstm_h1 = int(lstm_hidden['Layer1'])
    activ_l1 = lstm_hidden['activ_l1']
    activ_l2 = lstm_hidden['activ_l2']


    batch_size = 1
    #inputs = [None, tsteps, in_dim]
    input_1 = Input(shape=(tsteps, in_dim), batch_shape=(1, tsteps, in_dim))
    encoder = LSTM(lstm_h1, activation=activ_l1, return_sequences=False, stateful=True)(input_1)


    #Adding a dense layer
    #encoder = Dense(lstm_h1, activation='relu')(encoder)
    encoding_repeat = RepeatVector(tsteps)(encoder) #[None, in_dim] -> [None, tsteps, in_dim]
    decoder = LSTM(out_dim, return_sequences=True, activation=activ_l2, stateful=True)(encoding_repeat)
    ...
    #predictions = model_MLP(decoder)
    #out_3D = Reshape((tsteps, ))(predictions) #might have to reshape
    model = Model(input=input_1, output=decoder)
...
def lstm_multi_104b(lstm_hidden, in_dim, h1_dim, out_dim, tsteps):
    ...
    #predictions = model_MLP(decoder)
    #out_3D = Reshape((tsteps, ))(predictions) #might have to reshape
    model = Model(input=[input_1, h_inputs], output=decoder)
    model.compile(loss='binary_crossentropy', optimizer='adam')
...
def lstm_multi_105(lstm_hidden, in_dim, out_dim, tsteps):
    ...
    #encoder = Dense(lstm_h1, activation='relu')(encoder)
    encoding_repeat = RepeatVector(tsteps)(encoder) #[None, in_dim] -> [None, tsteps, in_dim]
    encoding_repeat = merge([encoding_repeat, input_1], mode='concat', concat_axis=2) #THis is what I want to input
    decoder = LSTM(out_dim, return_sequences=True, activation=activ_l2,  stateful=True)(encoding_repeat)
...
def lstm_multi_106(lstm_hidden, in_dim, out_dim, tsteps):

    lstm_h1 = int(lstm_hidden['Layer1'])
    activ_l1 = lstm_hidden['activ_l1']
    activ_l2 = lstm_hidden['activ_l2']
    lambda_1 = lstm_hidden['lambda_1']
    ...
    #predictions = model_MLP(decoder)
    #out_3D = Reshape((tsteps, ))(predictions) #might have to reshape
    model = Model(input=input_1, output=[out_1, out_2])
    model.compile(loss='binary_crossentropy', optimizer='adam', loss_weights=[lambda_1, 1-lambda_1])
...
def lstm_multi_107(lstm_hidden, in_dim, tsteps):
    ...
    encoding_repeat = RepeatVector(tsteps)(encoder) #[None, in_dim] -> [None, tsteps, in_dim]
    #encoding_repeat = merge([encoding_repeat, input_1], mode='concat', concat_axis=2) #THis is what I want to input
    decoder = LSTM(1, return_sequences=True, activation='sigmoid',  stateful=True)(encoding_repeat)
...
def lstm_multi_107b(lstm_hidden, in_dim, h1_dim, tsteps):
...
def lstm_multi_107e(lstm_hidden, in_dim, tsteps):
...
def lstm_multi_107f(lstm_hidden, in_dim, tsteps):
    ...
    #out_3D = Reshape((tsteps, ))(predictions) #might have to reshape
    model = Model(input=input_1, output=[out_1, out_2])
    model.compile(loss='mean_squared_error', optimizer='adam', loss_weights=[lambda_1, 1-lambda_1])
