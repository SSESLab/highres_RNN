...
H_t2 = DataFunctions.prepare_energy_PSB(crac_normal, std_inv2) #Energy Consumption Data for multi timescale analysis
...
if choice == 1:
    H_t = numpy.load('H1_file_CRAC2.npy')
elif choice == 2:
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_val2 = DataFunctions.prepare_energy_PSB(crac_normal, std_inv2) #Energy Consumption Data for multi timescale analysis
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_e2 = DataFunctions.prepare_energy_PSB(crac_normal, std_inv2) #Energy Consumption Data for multi timescale analysis
...
###Filling in data using H_t
y_B = numpy.load('CRAC_normal_B.npy')
y_temp = y_B.flatten()
print y_temp.shape
...
def objective(params):
    optimize_model = build_lstm_v1.lstm_multi_107f(params, train_data2.shape[2], (std_inv/std_inv2)) #Check code here, relu entering
...
#Building model
lstm_model = build_lstm_v1.lstm_multi_107f(lstm_hidden, train_data2.shape[2],  tsteps2)
...
for attempts in range(attempt_max):
    ...
    for ep in range(epoch_max):
        ...
        if val_loss_v[count] < epsilon and loss_val < loss_old:
            ...
            Y_val = test_model.predict([val_data2], batch_size=1, verbose=0)
            test_model.reset_states()
...
####Saving block
numpy.save('CRAC_normal_10min2F0.npy', Y_lstm)
numpy.save('CRAC2F_val0.npy', Y_val)
