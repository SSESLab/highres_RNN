...
file_ = r'/home/sseslab/Documents/dataport_data/dataport77.csv'
...
file_ = r'/home/sseslab/Documents/dataport_data/dataport77b.csv'
...
space2 = {
        ...
        'Layer2': hp.quniform('Layer2', 10, 100, 5),
        'Layer3': hp.quniform('Layer3', 5, 100, 1),
        'activ_l3': hp.choice('activ_l3', [ 'tanh', 'sigmoid']),
        'activ_l4': hp.choice('activ_l4', ['sigmoid', 'linear'])
...
get_2nd_layer_output = K.function([save_model.layers[0].input], [save_model.layers[1].output])
lstm_h2 = int(lstm_hidden['Layer2'])
h_input = numpy.zeros((len(train_data2), 60, lstm_h2))
h_val = numpy.zeros((len(val_data2), 60, lstm_h2))
h_test = numpy.zeros((len(test_data2), 60, lstm_h2))

for i in range(0, len(train_data)):
    X_temp = train_data[i, :, :]
    X_temp = X_temp[None, :, :]
    h_temp = numpy.asarray(get_2nd_layer_output([X_temp]))
    h_temp = numpy.reshape(h_temp, (24, lstm_h2))
    h_input[i*24:(i+1)*24, :] = h_temp
...
trials = Trials()
best2 = fmin(objective, space2, algo=tpe.suggest, trials=trials, max_evals=5)
