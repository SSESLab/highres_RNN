...
#Replicating data for 10-min resolution
x_t2 = numpy.zeros((len(H_t2), 1))


for i in range(0, len(H_t)):
    x_t2[i*(std_inv/std_inv2):(i+1)*(std_inv/std_inv2)] = H_t[i]

#train_data2 = numpy.concatenate((weather_train2[:, 0:2], X_sch_t2, x_t2), axis=1)
train_data2 = numpy.concatenate((weather_train2[:, 0:2], X_sch_t2, T_sch_t2, x_t2), axis=1)
...
#Replicating data for 10-min resolution
x_v2 = numpy.zeros((len(H_val2), 1))

for i in range(0, len(H_val)):
    x_v2[i*(std_inv/std_inv2):(i+1)*(std_inv/std_inv2)] = H_val[i]

#val_data2 = numpy.concatenate((weather_val2[:, 0:2], X_sch_val2, x_v2), axis=1)
val_data2 = numpy.concatenate((weather_val2[:, 0:2], X_sch_val2, T_sch_val2, x_v2), axis=1)
...
y_temp = y_B.flatten()
x_e2 = numpy.zeros((len(H_e2), 1))

for i in range(0, len(H_e)):
    x_e2[i*(std_inv/std_inv2):(i+1)*(std_inv/std_inv2)] = y_temp[i]

#test_data2 = numpy.concatenate((weather_test2[:, 0:2], X_sch_e2, x_e2), axis=1)
test_data2 = numpy.concatenate((weather_test2[:, 0:2], X_sch_e2, T_sch_e2, x_e2), axis=1)
...
def objective(params):
    optimize_model = build_lstm_v1.lstm_multi_105(params, train_data2.shape[2], H_t2.shape[2], (std_inv/std_inv2)) #Check code here, relu entering
...
#Building model
lstm_model = build_lstm_v1.lstm_multi_105(lstm_hidden, train_data2.shape[2], H_t2.shape[2], tsteps2)
...
numpy.save('CRAC_critical_10min2E.npy', Y_lstm)

e_deep = (MathFunctions.rms_flat(Y_lstm - Y_e2))/(MathFunctions.rms_flat(Y_e2))
e_deep2 = (MathFunctions.rms_flat(Y_lstm - Y_e2))/(MathFunctions.rms_flat(H_t2))
