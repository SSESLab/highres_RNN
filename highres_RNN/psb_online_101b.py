...
#numpy.save('Ht2_HVAC1.npy', H1)
#numpy.save('Hv2_HVAC1.npy', H2)
#numpy.save('He2_HVAC1.npy', H3)
...
for attempts in range(attempt_max):
    ...
    for ep in range(epoch_max):
        ...
        for i in range(0, int(len(val_data2)/t_factor)-1):
            X = val_data2[i*t_factor:(i+1)*t_factor, :, :]
            Y = H_val2[i*t_factor:(i+1)*t_factor, :, :]
            X2= val_data2[(i+1)*t_factor:(i+2)*t_factor, :, :]
            Y2 = H_val2[(i+1)*t_factor:(i+2)*t_factor, :, :]

            val_history = lstm_model.fit(X, Y, batch_size=1, nb_epoch=1, validation_split=0, shuffle=False)
            val_out = lstm_model.predict(X2, batch_size=1, verbose=0)

            v_out = DataFunctions.fix_bindata(val_out)
            v_out = DataFunctions.make_realdata(v_out, delta)

            v_a = DataFunctions.fix_bindata(Y2)
            v_a = DataFunctions.make_realdata(v_a, delta)

            e_val = (MathFunctions.rms_flat(v_out - v_a))/(MathFunctions.rms_flat(v_a))
            sum_val = sum_val + e_val

        # testing alternative block
        val_loss_check = sum_val/((len(val_data2)-1)/t_factor)
        ...
        if val_loss_v[count] < epsilon and loss_val < loss_old:
            ...
            save_model = lstm_model
            lstm_model.save_weights('multi_model_HVAC1.h5', overwrite=True)
...
lstm_model.load_weights('multi_model_HVAC1.h5')

sum_val = 0

for i in range(0, int(len(test_data2)/t_factor)-1):
    X = test_data2[i*t_factor:(i+1)*t_factor, :, :]
    Y = H_e2[i*t_factor:(i+1)*t_factor, :, :]

    test_out = lstm_model.predict(X, batch_size=1, verbose=0)
    test_history = lstm_model.fit(X, Y, batch_size=1, nb_epoch=1, validation_split=0, shuffle=False)

    test_out = DataFunctions.fix_bindata(test_out)
    test_out = DataFunctions.make_realdata(test_out, delta)

    test_a = DataFunctions.fix_bindata(Y)
    test_a = DataFunctions.make_realdata(test_a, delta)

    e_val = (MathFunctions.rms_flat(test_out - test_a))/(MathFunctions.rms_flat(test_a))
    sum_val = sum_val + e_val




e_on = sum_val/(len(test_data2)/t_factor-1)
print e_on
#numpy.save('HVAC_critical_10min2A.npy', Y_lstm)
#numpy.save('HVAC1_val.npy', Y_val)
