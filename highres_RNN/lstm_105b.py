...
H_t = DataFunctions.fix_data(conv_normal) #substract to find differences
...
if choice == 1:
    H_t = numpy.load('H1_file_CONV2.npy')
elif choice == 2:
    ...
    Y_t, Y_NN = InterpolateFunctions.interp_LSTM_v2(train_data, H_t, large_list)
    ...
    #H_t = Y_t.copy()
    H_t = Y_NN.copy()
    numpy.save('H1_file_CONV2.npy', H_t)
...
numpy.save('CONV_normal_train.npy', H_t)
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_val = DataFunctions.fix_data(conv_normal) #substract to find differences
...
numpy.save('CONV_normal_val.npy', H_val)
...
data = DataFunctions.read_PSB_csv(date_start, date_end, 5, 20) #read data at 5 min resolutions
conv_critical, crac_critical, crac_normal, conv_normal, HVAC_critical, Elev, HVAC_normal, elec_total = DataFunctions.PSB_elec_split(data) #split by end uses
H_e = DataFunctions.fix_data(conv_normal) #substract to find differences
...
numpy.save('CRAC_critical_test.npy', H_e)
...
space = {
        ...
        'activ_l3': hp.choice('activ_l3', ['relu', 'tanh', 'sigmoid']),
...
numpy.save('CONV_normal_B.npy', Y_lstm2)
numpy.save('CONV_normal_MLP.npy', Y_NN)


#### Plotting
PlotFunctions.Plot_double(t_array, H_e, t_array, Y_lstm2, 'Actual conv power','LSTM conv power', 'k-', 'r-', "figA_CRAC2a.eps")
PlotFunctions.Plot_triple(t_train, H_t, t_test, Y_lstm2, t_test, H_e, 'Training Data', 'LSTM predictions', 'Test Data (actual)', 'k-', 'r-', 'b-', "figA_CRAC2b.eps")
PlotFunctions.Plot_triple(t_train, H_t, t_test, Y_lstm, t_test, H_e, 'Training Data', 'LSTM predictions', 'Test Data (actual)', 'k-', 'r-', 'b-', "figA_CRAC2c.eps")
PlotFunctions.Plot_quadruple(t_train, H_t, t_test, Y_lstm2, t_test, Y_NN, t_test, H_e, 'Training Data', 'LSTM predictions', 'MLP Predictions', 'Test Data (actual)', 'k-', 'r-', 'y-', 'b-', "figA_CRAC2d.eps")

##Computing daily predictions:
Y_p_daily = DataFunctions.reduce_by_sum(Y_lstm2, 24)
H_e_daily = DataFunctions.reduce_by_sum(H_e, 24)

e_daily = (MathFunctions.rms_flat(Y_p_daily - H_e_daily))/(MathFunctions.rms_flat(H_e_daily))
sim_score = (MathFunctions.rms_flat(H_e - H_t[0:len(H_e)]))/(MathFunctions.rms_flat(H_e))
